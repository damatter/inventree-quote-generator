...
def test_remote_ui_exports_all_inventree_entry_points():
    script = (PACKAGE_ROOT / "static" / "inventree_quote_generator" / "ui.js").read_text(
        encoding="utf-8"
    )
...
def test_site_templates_and_assets_are_present():
    ...
    assert (PACKAGE_ROOT / "static" / "inventree_quote_generator" / "site.css").is_file()
    assert (PACKAGE_ROOT / "static" / "inventree_quote_generator" / "site.js").is_file()
